...
<template>
    <li ref="menuItemRef" :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        <div v-if="root && item.visible !== false" class="layout-menuitem-root-text text-sm">{{ item.label }}</div>
        <a
        ...
        >
            ...
            <font-awesome-icon :icon="['fas', item.icon]"  class="layout-menuitem-icon" />
            <span class="layout-menuitem-text">{{ item.label }}</span>
            <font-awesome-icon :icon="['fas', 'angle-down']"  class="layout-submenu-toggler" v-if="item.items" />
