...
<template>
    <div :class="['grid px-6', { 'rtl-direction': rtl }]">
        ...
        <div class="col-12 md:col-6 lg:col-4 xl:col-3" v-for="data in paginatedEntities" :key="data.id">
            <Card class= "min-w-15rem max-w-30rem h-25rem">
                <template #header>
                    <div class="w-full h-8rem bg-primary flex align-items-center justify-content-center">
                ...
                <template #title>
                    <div class="flex justify-content-between gap-1 align-items-center">
                        ...
                        <Tag :severity="!data.isDeActivated ? 'success' : 'danger'" :value="!data.isDeActivated ? 'Active' : 'Inactive'"></Tag>
                    </div>
                </template>
                <template #subtitle>
