                ...
                <div class="flex flex-column gap-4">
                    <div v-for="(item, index) in invoiceStore.invoice.items" :key="item.id" class="flex flex-row justify-content-between">
                        <div class="flex flex-column gap-1">
                            <span class="font-semibold text-900 text-lg">{{ item.product.name }}</span>
                            <div class="flex flex-row gap-2">
                                <span class="text-500">{{ item.selectedBatch.price }}x{{ item.quantity }}</span>
...
const subtotal = computed(() => {
    return invoiceStore.invoice.items.reduce((total, item) => {
        return total + item.selectedBatch.price * item.quantity;
    }, 0);
});
const discount = computed(() => {
    return invoiceStore.invoice.items.reduce((total, item) => {
        if (item.discount.type === 'percentage') return total + (item.selectedBatch.price * item.quantity * item.discount.value) / 100;
