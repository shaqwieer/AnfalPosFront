...
const filterBranches = computed(() => {
  if (roles.value != null && props.IsAdd && roles.value.some((role) => role.isVanSalesRep)) {
    return branchesOptions.value.filter((e) => e.organizationId === organization?.value?.id && !props.vanSalesRepBranches.includes(e.id));
  } else {
    ...
    <div class="flex flex-column gap-3 p-4">
      <div class="flex gap-2 border-1 p-4 border-round-lg overflow-scroll scroll-container">
        <div class="field flex flex-column w-full">
          ...
          <Dropdown v-model="country" v-bind="countryAttrs" :virtualScrollerOptions="{ itemSize: 38 }" :options="countries" filter :loading="false" optionLabel="name" :placeholder="t('CreateUpdateUsersRoles.countryPlaceholder')" class="w-full">
        ...
        <div class="field flex flex-column w-full">
          ...
          <MultiSelect
            ...
            :maxSelectedLabels="1"
            :disabled="selectedData?.roles?.some((role) => role.isVanSalesRep === true)"
            filter
            optionLabel="name"
        ...
        <div class="field flex flex-column w-full" v-if="IsAdd || !selectedData?.roles?.some((role) => role.isVanSalesRep === true)">
      ...
      <div class="flex gap-2 border-1 p-4 border-round-lg" v-if="IsAdd || !selectedData?.roles?.some((role) => role.isVanSalesRep === true)">
        <div class="field flex flex-column w-full">
